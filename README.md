...
Read and write Gmail from inside Vim. Each message is a folder of plain files
...
**One Gmail setting:** so that deleting mail in vim-mail actually deletes it,
go to Gmail → Settings → *Forwarding and POP/IMAP* → set *"When a message is
marked as deleted"* to **"Immediately delete the message forever"**.
...
- **Clipboard features are only tested on macOS.** `<leader>a` (clipboard files)
  and `<leader>p` (clipboard image) work out of the box on macOS with no extra
  tools. On Linux they need `xclip` or `wl-clipboard` installed and are currently
  **untested** — and may not pick up files copied from GNOME Files. Attaching by
