# vim-mail backend setup (msmtp + getmail + store)

The vim-mail plugin is a frontend over a one-folder-per-message mail store.
Underneath it needs three things wired up:

1. **Outbound** — msmtp submitting to Gmail SMTP, so `mail_store.py send`
   (called on `:w` from a compose buffer) actually delivers.
2. **Inbound** — getmail pulling new Gmail messages straight into the store
...
## 1. Outbound: msmtp → Gmail SMTP

### Goal
`mail_store.py send` pipes the assembled message to `msmtp -t`, which submits it
straight to `smtp.gmail.com:587` (STARTTLS), authenticated from `~/.msmtprc`. No
local MTA — no Postfix, no `/etc`, no sudo.

### Config
`~/.msmtprc` (mode `600` — msmtp refuses a world-readable file that holds a
password):

msmtp does **not** accept trailing comments on a value line (a `#` must start its
own line), so keep any notes above the directive:

```
defaults
auth on
tls on
# tls_trust_file: your platform CA bundle (macOS/BSD /etc/ssl/cert.pem;
# Debian/Ubuntu /etc/ssl/certs/ca-certificates.crt)
tls_trust_file /etc/ssl/cert.pem

# 465 implicit-TLS provider instead? use "port 465" and "tls_starttls off"
account gmail
host smtp.gmail.com
port 587
tls_starttls on
...
Verification). `./setup.sh` prints this block; `setup_lazyass.sh` writes it for
you. The transport is `$MAIL_SENDMAIL` (default `msmtp -t`) — set
`g:mail_send_cmd` to `sendmail -t` if you'd rather relay through a local MTA.

### Note — Gmail rewrites `From:`
Gmail's SMTP relay rewrites the `From:` header server-side to match the
authenticated account (an anti-spoofing measure), so a mismatched `From` in the
compose buffer is silently corrected, not rejected — you can't send as an
address you don't own. Verify a login without sending via
`msmtp --serverinfo --host smtp.gmail.com --port 587 --tls=on --tls-starttls=on`.

(The old Postfix relay — `/etc/postfix` + SASL, and the macOS SASL-mechanism
quirk it needed — is gone; git history has it if ever needed.)

## 2. Inbound: getmail → the store

```bash
brew install getmail6
```

A getmailrc (mode `600`, default `~/.getmail/getmailrc`) pulls each new message
and hands it to the store via an `MDA_external` — piping the raw bytes to
`mail_store.py ingest-stdin <inbox>`, instead of writing a Maildir/mbox. Mail
always lands in `inbox`; file it into other folders afterward.
...
arguments = ("/path/to/vim-mail/scripts/mail_store.py", "ingest-stdin", "/path/to/Mail/inbox")
...
The `path` + `arguments` MDA is the only part that lives outside the repo and
can't be auto-resolved. Run `./setup.sh` to print the exact block for this
machine, or `./setup.sh --patch` to rewrite an existing `~/.getmail/getmailrc`'s
MDA paths in place (backs up first; preserves the `ingest-stdin`/inbox args and
your password).
...
```bash
getmail                 # single poll
getmail --dump          # show what a run would do without retrieving
```

`read_all = false` fetches only messages new since the last run (getmail tracks
seen UIDs in an "oldmail" state file under its data dir); `delete = false` keeps
them on the server. This setup is **on-demand** (no daemon) — `<leader>f` runs
`getmail` asynchronously (rc from `g:mail_getmail_rc`) and refreshes the inbox
on completion.
...
via `msmtp -t`, the message is ingested into `<sent-dir>` (default
...
it via `msmtp -t` (direct to Gmail SMTP). The sent copy is automatically
...
| `tests/test_reply_integration.py` | Full pipeline on that real message: CLI ingest → real headless `vim` replies (top-post) **and forwards both ways** + sends (fake `msmtp`) → sent box verified — reply (multipart/alternative, tables embedded, cid as multipart/related, clean `>` quote, threading), inline forward (tables embedded, `.ics` re-attached, new thread), and as-attachment forward (`message/rfc822`, attachments intact). |
